...
if EXECUTION_TERMINAL == True:
  ...
  args = parser.parse_args(["--n_trials", "3", "--n_trials_sampling", "2", "--n_trials_pruning", "2", "--n_cross_val_hyperparam", "2",  #"--context",
                            ...
                            "--augmentation_nmt", "many2many",  # "no-nmt-single", "one2anchor", "one2many", "no-nmt-many", "many2manymany2many", "many2many"
...
def optuna_objective(trial, lang=None, n_sample=None, df_train=None, df=None):  # hypothesis_hyperparams_dic=None
  ...
  for step_i, random_seed_cross_val in enumerate(np.random.choice(range(1000), size=CROSS_VALIDATION_REPETITIONS_HYPERPARAM)):
    ...
    if "manifesto" in DATASET:
        ...
        df_train_split_ids, df_dev_ids = train_test_split(df_train.rn.unique(), test_size=test_size, shuffle=True, random_state=random_seed_cross_val)
    ...
    print("Language distribution for df_dev_scenario_samp: ", df_dev_scenario_samp.language_iso.value_counts())
    print("\n")
    ...
    print("Language distribution for df_train_scenario_samp_augment: ", df_train_scenario_samp_augment.language_iso.value_counts())
